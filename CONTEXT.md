...
A lightweight, Supabase-backed multi-tenant SMS gateway turning Android devices into cellular messaging relays managed via a web dashboard and HTTP API.
...
**Claim**:
The atomic database reservation of a pending Outbound Message by an active Gateway Device using row-level locking (`FOR UPDATE SKIP LOCKED`).
...
**Heartbeat**:
A periodic telemetry packet sent by a Gateway Device reporting battery level, network type, signal strength, and liveness.
_Avoid_: Ping, keep-alive, check-in.

**Pairing Session**:
A temporary, single-use, 10-minute cryptographic exchange initiated via QR code that binds a Gateway Device to an Organization.
